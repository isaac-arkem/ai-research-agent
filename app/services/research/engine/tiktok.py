...
def search_and_enrich(
    ...
    """
    # Apify instead of ScrapeCreators. The caller decides by clearing `token`
    # (pipeline did this via SOCIAL_PROVIDER=apify); an Apify token with no
    # ScrapeCreators token means Apify is the only way to answer, so take it.
    if apify_token and not token:
        from . import apify_social

        return apify_social.search_tiktok_apify(
            topic,
            from_date,
            to_date,
            depth=depth,
